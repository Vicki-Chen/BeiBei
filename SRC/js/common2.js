		...
		$.ajax({
			...
			success:function(str){
				...
				for(var i=0;i<data.length;i++){
					html += '<li><a href="yzzj.html"><img src="../'+data[i].img+'"/><p>'+data[i].name+'</p></a></li>';
				}
			...
			for(var i =1;i<7;i++){
				if(getCookie("id"+i)){
					...
					$.ajax({
						...
						success:function(str){
							var data = JSON.parse(str);
							$li = $('<li><div><img src="'+data[0].photo+'" alt=""></div><div><p>'+data[0].include+'</p><p>'+data[0].xuanze+'</span></div><div><p>￥'+data[0].price+'</p><p>x'+data[0].shuliang+'</p></div></li>');
